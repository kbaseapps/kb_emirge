...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'kb_emirge'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from kb_emirge.kb_emirgeImpl import kb_emirge
impl_kb_emirge = kb_emirge(config)
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'kb_emirge'
        ...
        self.rpc_service.add(impl_kb_emirge.run_emirge,
                             name='kb_emirge.run_emirge',
                             types=[dict])
        self.method_authentication['kb_emirge.run_emirge'] = 'required'
        self.rpc_service.add(impl_kb_emirge.exec_emirge,
                             name='kb_emirge.exec_emirge',
                             types=[dict])
        self.method_authentication['kb_emirge.exec_emirge'] = 'required'
        self.rpc_service.add(impl_kb_emirge.exec_emirge_SingleLibrary,
                             name='kb_emirge.exec_emirge_SingleLibrary',
                             types=[dict])
        self.method_authentication['kb_emirge.exec_emirge_SingleLibrary'] = 'required'
        self.rpc_service.add(impl_kb_emirge.status,
                             name='kb_emirge.status',
                             types=[dict])
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "kb_emirge but no authentication header was passed"
                            raise err
